...
<form method="post" class="input-form col-xs-10 col-xs-offset-1 col-sm-10 col-sm-offset-1 col-md-6 col-md-offset-3">
    ...
    <div class="row form-group">
        ...
        <input class="col-xs-8" id="Name" name="Name" type="text" value="{{ old('Name') }}" placeholder="Role Name"/> 
    </div>
    ...
    <div class="row form-group" style="max-height: 300px; overflow-y: auto; border: solid 1px black; border-radius: 5px; padding: 5px 0px;">
        ...
        <div class="col-xs-12 form-group">
            <label class="col-xs-2" for="{{ $access->Code }}"></label>
            <input class="col-xs-1" id="{{ $access->Code }}" name="{{ $access->Code }}" value="{{ $access->Id }}" type="checkbox" />
            <label class="col-xs-4" for="{{ $access->Code }}">{{ $access->Code }}</label>
            <label class="col-xs-4" for="{{ $access->Code }}">{{ $access->Name }}</label>
        </div>
        @endforeach
    </div>
